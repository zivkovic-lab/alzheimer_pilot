...
This datasets also contains some zeros/missing values. So again I added a prior of 1 to every value just for the purpose of letting the statistic analysis to preceed.

```{r, out.width="100%"}
(data.frame(
    feature = featureNames(abs_abund),
    abs_abund = apply(abs_abund$conc_table, 1, function(row){sum(row == 0)})
) %>% 
    ggplot() + 
...
Here I transformed the absolute abundance data into relative abundance. A pior of 1 is added before the transforming.

```{r}
abs_abund1 = transform_by_feature(abs_abund, function(x) x + 1)
rel_abund = transform_by_sample(abs_abund1, function(x) x/sum(x) * 100)
```


## 3. Normality

As shown below in the histogram of the z-scroe scaled median of each glycan, that it skews to the left, due to the zeros.

```{r, out.width="100%"}
(data.frame(
    glycan = featureNames(abs_abund),
    abs_abund = apply(abs_abund$conc_table, 1, function(x) median(scale(x )))
) %>% 
...
(data.frame(
    glycan = featureNames(abs_abund),
    abs_abund = apply(abs_abund$conc_table, 1, function(x) median(scale(log(x + 1))))
) %>% 
    ggplot(aes(x = abs_abund, y = ..density..)) +
    geom_histogram(color = "white", bins = 25, fill = "steelblue") +
    geom_density()) %>%
...
```{r}
pca1 = abs_abund1 %>% conc_table %>% log %>%  apply(1, scale) %>% prcomp
...
```{r}
pdata = as(abs_abund$sample_table, "data.frame")
pdata$interaction = interaction(pdata$region, pdata$group)
design = model.matrix(~ interaction + 0, data = pdata)
contrasts = paste0("interaction", unique(pdata$region), ".AD - interaction", unique(pdata$region), ".Control")

lm = lapply(list(abs_abund, rel_abund), function(mset){
...
names(lm) = c("abs_abund", "rel_abund")
```
...
output$dt = renderDataTable({
    datatable(
        cbind(abs_abund$feature_data, lm[[input$`data-type`]][[input$region]]),
...
renderPlotly({
    ...
    data.frame(
        ...
        region = mset$sample_table$region
    ) %>%
